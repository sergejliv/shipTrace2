...
func TestAddGetDelete(t *testing.T) {
	...
	// Заменяем на assert для проверки полей - тест может продолжиться даже если одно поле не совпадает
...
func TestSetAddress(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, newAddress, updatedParcel.Address) // assert для проверки значения
}
...
func TestSetStatus(t *testing.T) {
	...
	updatedParcel, err := store.Get(id)
	require.NoError(t, err)
	assert.Equal(t, ParcelStatusSent, updatedParcel.Status) // assert для проверки значения
}
...
func TestGetByClient(t *testing.T) {
	...
	for i := 0; i < len(parcels); i++ {
		...
		require.NoError(t, err)
		assert.NotZero(t, id) // assert для проверки ID
